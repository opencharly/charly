...
another, and raises `ValueError` when the command cannot be tokenized (a heredoc,
an unbalanced/unquoted quote). The caller MUST fail closed on that ValueError —
returning "no command" there is how a gate silently stops gating.
"""
import os
import re
import shlex

...
#   - cannot be TOKENIZED (a heredoc message, an unbalanced/unquoted quote): the
#     gate fails CLOSED and blocks it — re-issue in a form it can parse (a long
#     message in a file via `git commit -F msg.txt`, quotes balanced).
...
try:
    ...
    # A heredoc, or an unbalanced/unquoted quote — valid bash that shlex cannot
    # tokenize. NEVER treat this as "no command" (that is how a gate silently
    # stops gating). FAIL CLOSED: if a `git … commit` is plausibly present, block —
    # no fallback re-parse. Re-issue in a form the gate can tokenize (put a long
    # message in a file: `git commit -F msg.txt`; avoid heredocs / unquoted quotes).
    if re.search(r'(?:^|[\s;&|(])git\b[^\n]*\bcommit\b', cmd):
        block("cannot parse this command (a heredoc, or an unbalanced/unquoted quote), so the "
              "gate cannot verify the commit. Re-issue it in a tokenizable form — put a long "
              "message in a file and use `git commit -F msg.txt`, and keep quotes balanced.")
    sys.exit(0)
if not invocations:
    sys.exit(0)

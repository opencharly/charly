...
for globs, args in invocations:
    ...
    # diff-dependent checks there. If it is present but not a resolvable directory
    # (a $VAR, a nonexistent path), the diff cannot be inspected — fail CLOSED
    # (below), matching the gate's behaviour before this rewrite. Re-issue with a
    # literal absolute path; never `cd` (a subdirectory project root drops settings).
    d = dash_c_dir(globs)
    if d is not None:
        if os.path.isdir(d):
            commit_cwd = d

...
try:
    ...
    # Unparseable (heredoc / unbalanced quote) — never treat as "no push". FAIL
    # CLOSED: if a `git … push` is plausibly present, block; no fallback re-parse.
    # (F1: returning "no command" here is how a gate silently stops gating.)
    if re.search(r'(?:^|[\s;&|(])git\b[^\n]*\bpush\b', cmd):
        block("cannot parse this command (a heredoc, or an unbalanced/unquoted quote), so the "
              "gate cannot verify the push. Re-issue it in a tokenizable form (keep quotes "
              "balanced; run the push as its own command).")
    sys.exit(0)
...
for globs, args in invocations:
    ...
    # stripped). `main` / `refs/heads/main` are forbidden destinations. Bounded to
    # THIS invocation's segment, so `git push feat/x && git log main` never trips.

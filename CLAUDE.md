...
- Core is a plugin host: every capability — verb, kind, deploy, step, builder, command — is a plugin candy importing only the SDK; in-core capability code is tracked K-wave inventory, never permanent, asserted by the P16 triple gate (which no workflow runs) (the import-surface assertion `charly/import_purity_test.go` — every `charly/` file imports only `spec/*` + the proto/plugin-API contract, zero `github.com/opencharly/sdk`; zero-aliases; per-plugin import hygiene — the former per-file allowlist is retired). *Detail:* `/charly-internals:plugin`.

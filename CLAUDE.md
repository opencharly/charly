...
- **R9 — Binary equals source.** Build the CalVer-stamped worktree-local binary with `task build:binary` and invoke it through that worktree's `bin/`; verify `charly version`. Runtime OS dependencies belong in the charly candy's `packaging:` section (`packaging/charly.yml`).

...
// del_resolve.go — the `charly bundle del` target resolution, relocated from the deleted
// charly/bundle_add_cmd.go's deployDelCmd + the deleted charly/host_build_deploy_del_resolve.go
...
func resolveDelNode(name string, tree map[string]spec.BundleNode) (*spec.BundleNode, string, error) {
	if name == "host" {
	...
	return nil, "", fmt.Errorf("no such deployment %q — run `charly bundle list` to see "+
		"deployments (a VM deploy is torn down as `charly bundle del vm:%s`)", name, name)
}

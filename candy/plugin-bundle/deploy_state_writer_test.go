...
// deploy-state cluster (charly/deploy_save_test.go's remaining SaveBundleConfig tests,
// charly/deploy_preserve_test.go — team-lead directive, 2026-08-03 split-by-assertion round).
// Each test here asserts deploykit.SaveBundleConfig / SaveVmDeployState / RemoveVmDeployEntry's
...
// charly/bundlenode_disposable_loader_test.go.
...
// TestSaveBundleConfig_AtomicWriteLeavesNoTempLeftover pins the tempfile + rename atomic-write
...
// TestSaveBundleConfig_RefusesToClobberUnloadableConfig pins the per-host persist fail-safe:
// when the caller's read callback reports the on-disk config currently FAILS to load, SaveBundleConfig
...
// SaveBundleConfig's OWN abort-on-read-error contract with a literal erroring reader, exactly
// mirroring this package's sibling TestSaveBundleConfig_ErrorsWhenCallbackNil-style fixtures.)
func TestSaveBundleConfig_RefusesToClobberUnloadableConfig(t *testing.T) {
	...
	erroringRead := func() (*deploykit.BundleConfig, error) {
		return nil, errFixtureUnloadable
	}

	err := deploykit.SaveBundleConfig(&deploykit.BundleConfig{Bundle: map[string]spec.BundleNode{
	...
	if err := deploykit.SaveBundleConfig(&deploykit.BundleConfig{Bundle: map[string]spec.BundleNode{
		"new-entry": {Target: "pod", Image: "new-entry"},
	}}, bedTestMarshalNode, bedTestLoadBundleConfig); err != nil {
		t.Fatalf("SaveBundleConfig with a healthy reader should succeed: %v", err)
	}
	...
	if _, ok := dc.Bundle["new-entry"]; !ok {
		t.Errorf("clean save did not persist new-entry; got keys %v", bundleTestKeysOf(dc.Bundle))
	}
}
...
// TestBundleNode_DisposableFalseRoundTrip_Writer pins the *bool Disposable WRITE-side fix: an
...
// charly-side companion test, charly/bundlenode_disposable_loader_test.go, proves the LOAD side:
// LoadUnified parsing nil/&false/&true correctly from a checked-in testdata fixture.)
func TestBundleNode_DisposableFalseRoundTrip_Writer(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("XDG_CONFIG_HOME", dir)
	if err := os.MkdirAll(filepath.Join(dir, "charly"), 0o700); err != nil {
		t.Fatalf("mkdir: %v", err)
...
func TestCharlyUpdatePreservesPerHostDeployFields(t *testing.T) {
	...
	node, ok := dc2.Bundle["vm:cachyos-gpu"]
	if !ok {
...
func TestVmDestroyRemovesPureAutoEntry(t *testing.T) {
	...
	if _, ok := dc2.Bundle["vm:check-cachyos-gpu-vm"]; ok {

...
func (c *BundleAddCmd) compileNodePlans(target, refStr, tag, path string, addCandies []string, vmEntity, builderImageOverride string) ([]*spec.InstallPlan, string, []string, error) {
	dir := c.dir
...
func (c *BundleAddCmd) compileRefSelection(ref *DeployRef, hostCtxJSON []byte, tag, vmEntity, dir string) ([]*spec.InstallPlan, string, []string, error) {
...
func (c *BundleAddCmd) compileAddCandyOnBox(alRef *DeployRef, baseBoxRef string, hostCtxJSON []byte, tag, dir string) ([]*spec.InstallPlan, error) {
...
// former host charly/bundle_add_cmd.go detectHostContext (vmshared.DetectHostDistro/DetectHostGlibc
...
// printPlans renders the compiled plans for a --dry-run — plugin-side (command:bundle is compiled-in,

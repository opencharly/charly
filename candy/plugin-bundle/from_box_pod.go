...
// from_box_pod.go — the POD path of `charly bundle from-box <ref> [name]`, relocated from the
// deleted charly/bundle_from_box_cmd.go + charly/host_build_deploy_from_box.go (K-wave 2 cone R2
...
// cp-box'd image but no project, so the host orchestrates `ssh guest 'charly bundle from-box <ref>
...
func runFromBoxPod(c *BundleFromBoxCmd) error {

...
// from_box_pod_test.go — the pod path of `charly bundle from-box`, relocated from the deleted
// charly/bundle_from_box_cmd.go (K-wave 2 cone R2 bank B). runFromBoxPod reaches deploy:pod's
...
func TestRunFromBoxPod_EmptyRefErrors(t *testing.T) {
	if err := runFromBoxPod(&BundleFromBoxCmd{}); err == nil {
...
func TestRunFromBoxPod_NameOverride(t *testing.T) {
	...
	if err := runFromBoxPod(&BundleFromBoxCmd{Ref: "ghcr.io/opencharly/redis:7", Name: "my-redis"}); err != nil {
		t.Fatalf("runFromBoxPod: %v", err)
	}
...
func TestRunFromBoxPod_QuadletStartsService(t *testing.T) {
	...
	if err := runFromBoxPod(&BundleFromBoxCmd{Ref: "ghcr.io/opencharly/redis:7"}); err != nil {
		t.Fatalf("runFromBoxPod: %v", err)
	}

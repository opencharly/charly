...
// BundleNode (with nested peer Members) as spec.CheckBedReply.NodeJSON, and this helper calls
...
// candy/plugin-bundle already use over the PERMANENT HostBuild("loader-threaded") leg), so the
...
// candy/plugin-deploy-pod's fetchLoaderPrimaries resolves. deploykit.MarshalBundleNode resugars
...
// registry-derived D-fact the former host marshalDeployNode fed to deploykit.MarshalBundleNode via
// loaderThreaded().Primaries. Sourcing Primaries PLUGIN-SIDE is what lets the bed persist run here
// instead of over a host seam.
func bedMarshalNode(ctx context.Context, ex *sdk.Executor) func(name string, node *deploykit.BundleNode) (*yaml.Node, error) {
...
// its load-mutate-save — the cycle-free loaderkit.LoadHostBundleConfigViaExecutor read, so the write
// needs no charly-init DeployStateHost.
func bedConfigReader(ctx context.Context, ex *sdk.Executor) func() (*deploykit.BundleConfig, error) {

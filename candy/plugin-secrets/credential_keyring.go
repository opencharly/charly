...
func (k *KeyringStore) Probe() error {
	...
	case <-time.After(keyringTimeout):
		...
		return fmt.Errorf("keyring probe timed out — unlock your keyring, or start a Secret Service provider")
	}
}
...
func (e *KeyringLockedError) Error() string {
	return fmt.Sprintf("keyring is locked (cannot %s %s/%s) — unlock your keyring to continue", e.op, e.service, e.key)
}

...
// command.go), never through the gRPC provider registry — so Invoke is unreachable. It
// exists only to satisfy the dual-mode sdk.Main signature (the shared NewMeta in plugin.go
// advertises NO capability, shipping only the doc schema for the load gate) — mirrors
// candy/plugin-udev / plugin-tmux / plugin-preempt / plugin-feature / plugin-doctor.

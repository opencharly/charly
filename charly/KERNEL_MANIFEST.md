...
| `main_repo.go` | 36 | K1-IOU | `ResolveProjectRepo` cache-path helper — feeds the git-clone/cache fetch mechanism, reached since K-wave 2 cone R1 through `requireProjectLoader().EnsureRepoDownloaded(ctx, …)` (the former `refs.go` core wrapper is DELETED: charly no longer assembles the fetch legs, candy/plugin-loader builds them itself off the ctx-threaded executor). The helper's own body is pure spec (`NormalizeRepoSpec` + `GitDefaultBranch`) over that one seam call, so it relocates with the rest of the loader facade, not ahead of it. |

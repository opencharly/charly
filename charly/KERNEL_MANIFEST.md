...
| File | LOC | Clause | Evidence |
|---|---:|---|---|
| `host_build_buildengine.go` | 223 | MIXED: M (registry-connect) / B (embed) / K1-IOU (remainder) | `hostBuildConnectPlugins` calls `loadProjectPlugins`, registering project build-time plugins into `providerRegistry` — M, the plugin-loading mechanism itself. `hostBuildContextIgnoreBaseline` returns `baselineContextIgnore`, parsed HERE (folded in from the deleted generate.go, K-wave 2 cone R1) from charly's OWN `//go:embed charly/charly.yml` — B, a separate Go module/binary structurally cannot embed another module's file. `hostBuildScanLocal` wraps the local candy scan (LoadUnified + ApplyDiscover + projectCandiesScanned) and `hostBuildNamespaced` calls `loadProjectForResolve`/`projectCandiesScanned`/`CollectRemoteRefsOpts` — K1-IOU, blocked on the load-entry resolution, not independently K3-movable. K-wave 2 cone R1 (A2) DELETED THREE legs. `buildengine-collect-remote-refs` re-derived cfg + the local scan host-side purely to hand them straight back to `loaderkit.CollectRemoteRefsOpts` (inputs candy/plugin-build already holds), and `buildengine-ensure-repo` was a bare forward to `loaderkit.EnsureRepoDownloaded` — both CALLS of a relocated mechanism, R-items by the defines-vs-calls test; the plugin drives them over `loaderkit.RefsSeamsFromExecutor` (the ONE shared executor-backed refs-legs builder) with `spec.BoxResolveOpts`/`spec.WithLocalRawRefs` in the shared fabric rather than duplicated across the boundary. `buildengine-scan-remote` was the LAST ScanSeams leg still crossing: its per-candy manifest parse APPEARED to need the clause-B `buildCandy` factory, which an RDD spike over the whole 324-manifest corpus DISPROVED — the pre-move `pn->genericNode->pn` round trip through it was an identity (321 node-form manifests plus all 3 error paths, byte-identical). The parse mechanism therefore relocated to `loaderkit.ParseCandyManifest` and the leg died; `buildCandy`/`candyIsImage` stay core for their GENUINE clause-B consumers (the discovered-candy pre-check, `foldCandyKind`). The earlier `hostBuildPrep` (`buildengine-prep`) leg is DELETED too: it existed only to populate the render-seam's Generator cache, which has no readers now that every render seam peer-dispatches. |

...
| `planrun_adapter.go` | 151 | M — wire-broker rendezvous | Defines `hostCheckCarrier`/`hostVerbResolver`, constructed at `plugin_dispatch_reverse.go:156,165` — the reverse dispatch server for out-of-process `InvokeProvider` calls. K-wave 2 cone R4 also relocated the do-mode/context grammar here from checkspec.go (`opEffectiveContexts`/`opInContext` — the functions this file's own header + checkrun.go's already named as this seam's grammar). |
...
| `checkrun.go` | 84 | E — generic result-envelope helpers + committed-APK anchoring data | K-wave 2 cone R4 relocated `candyDirsFromScan` + the `checkRunnerContext` carrier here from check_cmd.go (its 108-LOC thin) — the data checkrun_charly_verbs.go's `resolveCheckApk` reads off `h.cc.CandyDirs()`/`CandyScanErr()`, and resolveCheckRunnerContext folds into the check-load-plugins side effect. The prior R1 correction still holds: the scoping map's "RunModeLive/RunModeBox, wide fan-in 22+ files" evidence is STALE (W0 deleted those aliases); the verdict (STAY, not worth moving) is unchanged. |
...
| `layers.go` | 191 | B/M — the BOOTSTRAP candy scan | **Clause CHECKED and CORRECTED (K-wave 2 cone R1 unit C).** By the defines-vs-calls test this file DEFINES no mechanism: `scanCandyFromLocal` is a bare forward into `requireProjectLoader().ScanCandyFromLocal`, and `scanSeamsFor` only builds the host-coupled closures that seam takes. What keeps it kernel is WHO CALLS it. Of the seven production callers of `ScanAllCandyWithConfigOpts`, three are the plugin BOOTSTRAP path — `plugin_loader.go` (loadDeployPlugins + hostBuildConnectPlugins), `plugin_prescan.go`, `provider_command_external.go` — and the host must scan candies to FIND AND CONNECT plugins before any plugin exists to be asked, the same irreducible-bootstrap argument that keeps `candyIsImage`/`buildCandy` core (clause B). The remaining four (`check_cmd.go`, `oci_step_emit.go`, `host_build_buildengine.go`, `host_build_check_load_plugins.go`) are R-items belonging to the check/build/oci cones — but relocating them removes CALLERS, not lines from this file, so they do not shrink it. Nothing honestly leaves in this cone: the row amends from DELETE(0) to **THIN(191)**.
...
- **`host_build_pod_config.go` (107) + `host_build_pod_config_seams.go` (79) + `devices.go` (55) + `sidecar.go` (43) + `gpu_shim.go` (80→28): DELETED/thinned (K-wave 2 cone R3 — the B6 per-leg death list, w3-adjudication.md item 3).** The "pod-config-setup"/"pod-config-remove" forwarders died first (candy/plugin-pod dispatches deploy:pod's OpConfigSetup/OpConfigRemove peer-to-peer, threading HostEnvJSON as DATA). The detect-devices leg became candy/plugin-deploy-pod's `detectDevices` (peer `InvokeProvider(verb:gpu)`, with `LogDetectedDevices` relocated from the deleted `devices.go`); the sidecar embed MOVED INTO candy/plugin-deploy-pod's own `go:embed` (sidecar_embedded.go, the tailscale template byte-identical to the former charly.yml copy) — the "pod-config-detect-devices"/"pod-config-list-sidecars" seams + their CUE wire types are gone. `gpu_shim.go` THINNED to the `DetectVFIO` exception leg (28 LOC, EXCEPTION-GPU clause below).
...
- **`host_build_check_bed.go` (407 LOC) + `check_bed_run.go` (~110 LOC) + `bundle_members.go`
  ...
  refuted (the boundary law's own named trap: a stays-core header is a claim, not a verdict — and
  `host_build_arbiter_bracket.go` itself has since been DELETED, K-wave 2 cone R2 bank E: the
  deploy-dispatch Start/Stop bracket now Invokes verb:arbiter peer-to-peer and the
  CHARLY_PREEMPT_LEASE guard lives in candy/plugin-preempt) —
...
| `loader_threaded.go` | 399 | M — the loader/refs/scanner seam registry + its LOAD-ENTRY forwards | Holds the registered spec.DocParser / ProjectWalker / ProjectLoader / CandyScanner / Materializer handles, the registry-derived `loaderThreaded()` D-snapshot, and the WalkSeams/MaterializeSeams the plugin calls back through — the plugin-loading + prescan-dispatch mechanisms themselves. K-wave 2 cone R1 COLOCATED the project LOAD-ENTRY forwards here from the deleted config.go / format_config.go / main_repo.go / unified.go: `LoadUnified`, `LoadConfig`, `LoadBuildConfigForBox`/`LoadDefaultBuildConfig`, `ResolveProjectRepo`, `ErrNoCharlyYml`, and (unit 3) `ApplyDiscover` + `projectCandiesScanned`. Each is a one-or-two-line forward into the seam declared in this same file, so the seam is their owner; inlining them into ~40 call sites instead would have GROWN the kernel to make four files disappear. Everything in those files that carried real LOGIC left to candy/plugin-loader instead — most recently `canonicalRef`, whose import-ref resolution is now `loaderkit.CanonicalRef` behind the `ProjectLoader.CanonicalRef` seam, leaving `hostWalkSeams`' `ResolveRef` a three-line closure. |
...
| `materialize.go` | 188 | B | materialize leaf legs (bootstrap-candy-routed fold + embedded-default parse) — corrected post K1-unit-1; the box⊻layer routing predicate `candyIsImage` (`node_candy.go`, below) is its sibling. (audit-prod-report.md) | K-wave 2 cone R1 unit C ADDED `materializeNodeInto` + `materializeProject` here from node_parsed.go (145->188): this file already held the `MaterializeProjectSeams` wiring both reach the registered Materializer through, so it is their owner. Colocation, not growth — the pair's LOC left node_parsed.go.
| `node_candy.go` | 56 | B | `candyIsImage`/`buildCandy` — the box⊻layer bootstrap factory; the discovered-candy pre-check calls it directly, so it stays *genericNode-typed permanently (own header, corroborated). (audit-prod-report.md) |
...
| `node_desugar.go` | 71 | D + M — the plugin-verb primary registry | **Clause CHECKED and CORRECTED (K-wave 2 cone R1 unit C); the old "parse-time plugin-verb sugar desugar" reading was wrong about what this file contains.** The desugar MECHANISM is not here — it relocated to `sdk/loaderkit/parse.go` (`desugarEntityPlan`/`desugarStep`) and reads primaries as DATA off the `spec.Threaded` snapshot. What remains is the host TABLE that snapshot is built from: `pluginPrimaries` is kind-recognition DATA consulted by word (clause D — `loaderThreaded()` projects it into `spec.Threaded.Primaries` before every parse) and `registerPluginPrimary` mutates it from the provider registry at capability registration (clause M). Neither half moves without the registry, so this file is NOT K1-IOU residue and its ledger row amends from DELETE(0) to **THIN(71)** — nothing honestly leaves. Unit C also found and DELETED a byte-identical dead COPY at `sdk/kit/plugin_primary.go`: its header claimed charly's two registration call sites called `kit.RegisterPluginPrimary` directly, which they never did (import purity forbids charly importing sdk/kit), so the K4 relocation was authored but never wired and left two SEPARATE mutable maps of one registry. Zero consumers anywhere; `GOWORK=off go build ./kit/...` green without it.
| `node_parsed.go` | 100 | B — the genericNode bridge | Was 133. K-wave 2 cone R1 unit C COLOCATED the materialize plumbing (`materializeNodeInto`'s accumulator seed/copy-back + `materializeProject`'s per-document loop) onto `materialize.go`, which owns every other host-coupled materialize leg AND `hostMaterializeProjectSeams`, the seam those two are reached through. What is left is only the conversion between the wire-safe `spec.ParsedNode` and the `*genericNode` that the bootstrap-critical `candyIsImage`/`buildCandy` routing (clause B) and `validateKindValueCUE`'s raw-discValue shape check still need, plus `normalizeNodeInto`, the test-facing bridge entry. Row amends from DELETE(0) to **THIN(100)**.
| `node_build.go` | 21 | B | own header: kept ONLY for `node_candy.go`'s `buildCandy` (bootstrap-critical) — "the discovered-candy pre-check (now materialize.go's fold path) calls it directly, so it stays *genericNode-typed permanently" (self-declared B). The former entity-body assembler itself already relocated to `sdk/loaderkit.AssembleEntityBody` (K1 unit 3b); what remains is the bootstrap-coupled wrapper. Shrank 53->21 LOC since the audit. (own header, corroborated — OVERRIDES the audit's stale R/K1 call) |
...
| `bundle_add_cmd.go` | 73 | M — deploy-dispatch host floor | `deriveChildExecutorForPath` (registry-coupled — `deployTraitDescent` needs the providerRegistry) + `loadConfigForDeploy` (build-shared — LoadConfig → LoadUnified, K1-loader-family-coupled, also reached by build_overlay.go). K-wave 2 cone R2 bank C thinned 214 → 73: the DEL resolution (`deployDelCmd`/`resolveDelNode`/`podDeploymentArtifactExists`) moved to `candy/plugin-bundle/del_resolve.go` with the "deploy-del-resolve" HostBuild seam DELETED. |
| `check_cmd.go` | 108 | M — plugin-loading mechanism (the check-load-plugins face) | K-wave 2 cone R4 thinned 136 → 108: `candyDirsFromScan` + `checkRunnerContext` moved to checkrun.go (the committed-APK anchoring data home). What remains is `resolveCheckRunnerContext`, called by host_build_check_load_plugins.go:39 (STAY) — the core-side half that scans candies + calls `loadProjectPlugins` into THIS process's providerRegistry (the M mechanism a plugin cannot perform for itself). |
| `checkspec.go` | 49 | M — the provider-registry act-capability read | K-wave 2 cone R4 thinned 107 → 49: `opEffectiveContexts`/`opInContext` moved to planrun_adapter.go (the do-mode/context grammar's documented home). What remains is `opActsInBuildDeploy`, called by validate_project_host.go:80 (the STAY "validate-word-sets" leg) — the registry-coupled act-capability predicate (ResolveStep/ResolveVerb + ProvisionActor/TypedStepProvider/BuildEmitter type-asserts) the validate plugin's envelope-derived word inventory is classified against. |
...
| `deploy_target_dispatch.go` | 46 | M — wire-broker reverse-channel leg | `dispatchDeployTarget` core-side S3b half — threads a live executor onto the ctx via the compiled-in in-proc reverse channel pattern and Invokes command:bundle's OpDeployDispatch. K-wave 2 cone R5 landed the A9 interface-contract move: the `UnifiedDeployTarget`/`LifecycleTarget` contracts relocated to spec/spec/deploy_target_unified.go (their options repointed to the CUE-sourced DeployTarget* types) — this dispatch leg stays core-resident (it bridges live same-process facts: the venue executor + build context), its former R-RESIDUE exit satisfied. (audit-prod-report.md; w3-adjudication.md item 2) |
...
| `host_build_deploy_node_del_dispatch.go` | 35 | M — registry-coupled terminal step | `ResolveTarget` + `target.Del` honoring the teardown gates — the K4-C walk port's del terminal. K-wave 2 cone R2 bank C thinned 56 → 35: the plugin resolves the node plugin-side (candy/plugin-bundle/del_resolve.go) + strips the "vm:" prefix, so the seam's resolveNode default + SplitVmAddress handling moved out. |
...
| `host_build_resolve_target_add.go` | 52 | M — registry-coupled terminal step | `ResolveTarget` + `utgt.Add` + `loadConfigForDeploy` (LoadUnified) + the ancestor executor chain — the K4-C SHAPE-2 per-node add terminal. K-wave 2 cone R2 bank D thinned 126 → 52: the plan re-materialization (spec.PlansFromViews), EmitOpts construction (spec.EmitOptsFromResolveTargetAdd), and the ancestor loop (spec.ReconstructParentExec over the registry-coupled deriveChildExecutorForPath) moved to spec helpers. The "config-resolve" HostBuild seam is DELETED (its three consumers self-serve via loaderkit — plugin-vm's hostConfigResolve, plugin-deploy-vm's resolvePriorVmState, plugin-kube's hostConfigResolveVmState). |
...
| `init_resolve.go` | 25 | M — kind-dispatch callback wrappers | `resolveInitConfigViaPlugin`/`invokeInitResolve` call `providerRegistry.ResolveKind("init")` + `invokeTyped` — the SAME `InvokeProvider`/registry kind-dispatch mechanism (leg 2) as `resource_resolve.go`/`distro_resolve.go`'s already-documented "M — kind-dispatch callback wrappers" rows above. Colocated here from the deleted `service_render.go` (K-wave 2 cone R2), which held the egress family + this init leg; the egress family moved to `candy/plugin-bundle/egress.go`. |
...
| `commands.go` | 38 | B/M — same-process stdio fact + the pod-update dispatch struct | `isTerminal`/`defaultIsTerminal` (the TTY-detection host util, STAY-called at `host_build_pod_lifecycle_dispatch.go:126-127` — the shell's stdio TTY state is a live same-process fact that only resolves in the process where the shell runs, clause B) + `podUpdateCmd` (the host-side `charly update` dispatch struct, constructed at `host_build_pod_lifecycle_dispatch.go:186`, dispatched via `update_deploy_dispatch.go`'s M `dispatchByDeployTarget`). K-wave 2 cone R6 thinned 96→38: the dead `containerExists` duplicate of `kit.ContainerExists` is DELETED (zero callers anywhere) and the Cutover-B "DELETED"-narrative comment blocks are trimmed to the two live symbols' docs. |
...
- **K1-IOU** — blocked on the K1 loader wave's OWN remaining core files (`LoadConfig`/
  `LoadUnified`, whose former core homes config.go/unified.go are now deleted) resolving further; not independently movable by a

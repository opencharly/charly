...
| File | LOC | Clause | Evidence |
|---|---:|---|---|
| `host_build_buildengine.go` | 258 | MIXED: M (registry-connect) / B (bootstrap-scan + embed) / K1-IOU (remainder) | `hostBuildConnectPlugins` calls `loadProjectPlugins`, registering project build-time plugins into `providerRegistry` — M, the plugin-loading mechanism itself. `hostBuildScanLocal`/`hostBuildEnsureRepo`/`hostBuildScanRemote` wrap the bootstrap-delicate local candy scan + `EnsureRepoDownloaded` (refs.go, the K1-floor git clone/cache) — B. `hostBuildContextIgnoreBaseline` returns `baselineContextIgnore`, parsed HERE (folded in from the deleted generate.go, K-wave 2 cone R1) from charly's OWN `//go:embed charly/charly.yml` — B, a separate Go module/binary structurally cannot embed another module's file. `hostBuildCollectRemoteRefs`/`hostBuildNamespaced` call `LoadConfig`/`CollectRemoteRefsOpts`/`ScanAllCandyWithConfigOpts` (all core-only per W1's config.go verdict) — K1-IOU, not independently K3-movable. The former `hostBuildPrep` (`buildengine-prep`) leg is DELETED: it existed only to populate the render-seam's Generator cache, which has no readers now that every render seam peer-dispatches. |
...
|---|---:|---|---|
| `validate.go` | 181 | K1-IOU | **The former "HOST's SPLICED cross-plugin CUE schema" justification for this row was REFUTED and is corrected here (R1 doc-divergence).** `coreCueSchema()` returned `{cueSchemaCtx, sharedCueSchema, cueKindDef}`, and `sharedCueSchema` was ONLY `schemaconcat.ConcatSchema(sdkschema.FS)` — the BASE schema. No plugin fragment was ever unified into it: the plugin splice is a SEPARATE value built in `plugin_loader.go` (`cueSchemaCtx.CompileString(baseBody + servedCUE)`) and used solely to gate that plugin's authored input. So there was no non-marshalable cross-plugin graph pinning these validators to the host, and no new seam family was needed — ruling 1 simply moved the base schema to the loader that validates against it. `validateCandyCUESchemas`/`validateProjectCUESchemas`/`validateRemoteCandies` fold into `candy/plugin-box` (which already owns `runValidateEngine` and imports loaderkit) — the NAMED EXIT this residue row carries; the clause token stays the manifest's established residue-with-named-exit marker. |

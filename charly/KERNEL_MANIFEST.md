...
| File | LOC | Clause | Evidence |
|---|---:|---|---|
| `host_build_buildengine.go` | 220 | MIXED: M (registry-connect) / B (embed) / K1-IOU (remainder) | `hostBuildConnectPlugins` calls `loadProjectPlugins`, registering project build-time plugins into `providerRegistry` — M, the plugin-loading mechanism itself. `hostBuildContextIgnoreBaseline` returns `baselineContextIgnore`, parsed HERE from charly's OWN `//go:embed charly/charly.yml` — B, a separate Go module/binary structurally cannot embed another module's file. `hostBuildNamespaced` recurses the import-namespace tree via `loadProjectForResolve`/`projectCandiesScanned`/`CollectRemoteRefsOpts` — K1-IOU, the LAST buildengine leg with a live body, and now the only one blocked on the load-entry resolution. K-wave 2 cone R1 (A2) DELETED FOUR legs, taking the family from 7 registered kinds to 3. `buildengine-collect-remote-refs` and `buildengine-ensure-repo` (unit 1) only CALLED a relocated mechanism on the plugin's behalf — R-items by the defines-vs-calls test; candy/plugin-build drives them over `loaderkit.RefsSeamsFromExecutor`, with `spec.BoxResolveOpts`/`spec.WithLocalRawRefs` in the shared fabric rather than duplicated across the boundary. `buildengine-scan-remote` (unit 2) and `buildengine-scan-local` (unit 3) both existed because the per-candy manifest parse APPEARED to need the clause-B `buildCandy` factory; an RDD spike over the whole 324-manifest corpus DISPROVED that — the pre-move `pn->genericNode->pn` round trip through it was an identity (321 node-form manifests plus all 3 error paths, byte-identical). The parse relocated to `loaderkit.ParseCandyManifest` and the scan body to `loaderkit.ProjectCandiesScanned`, so a plugin holding its own `uf` (whose walk already runs `discover:`) scans without the host. `buildCandy`/`candyIsImage` stay core for their GENUINE clause-B consumers (the discovered-candy pre-check, `foldCandyKind`). The earlier `hostBuildPrep` (`buildengine-prep`) leg is DELETED too: it only populated a render-seam Generator cache with no readers. |

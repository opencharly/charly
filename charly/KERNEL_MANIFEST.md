...
| File | LOC | Clause | Evidence |
|---|---:|---|---|
| `host_build_buildengine.go` | 224 | MIXED: M (registry-connect) / B (bootstrap-scan + embed) / K1-IOU (remainder) | `hostBuildConnectPlugins` calls `loadProjectPlugins`, registering project build-time plugins into `providerRegistry` — M, the plugin-loading mechanism itself. `hostBuildScanLocal`/`hostBuildScanRemote` wrap the bootstrap-delicate local candy scan + the registry-coupled per-candy remote manifest parse (`parseCandyYAML`, threading the registered `DocParser` + the `Threaded` snapshot) — B. `hostBuildContextIgnoreBaseline` returns `baselineContextIgnore`, parsed HERE (folded in from the deleted generate.go, K-wave 2 cone R1) from charly's OWN `//go:embed charly/charly.yml` — B, a separate Go module/binary structurally cannot embed another module's file. `hostBuildNamespaced` calls `loadProjectForResolve`/`projectCandiesScanned`/`CollectRemoteRefsOpts` (core-only per W1's config.go verdict) — K1-IOU, not independently K3-movable. K-wave 2 cone R1 (A2) DELETED two more legs: `buildengine-collect-remote-refs` and `buildengine-ensure-repo`. Both only CALLED a relocated mechanism on the plugin's behalf — collect-remote-refs re-derived cfg + the local scan host-side purely to hand them straight back to `loaderkit.CollectRemoteRefsOpts` (inputs candy/plugin-build already holds), and ensure-repo was a bare forward to `loaderkit.EnsureRepoDownloaded` — which the defines-vs-calls test classifies as R-items. The plugin now drives both over `loaderkit.RefsSeamsFromExecutor` (the ONE shared executor-backed refs-legs builder), and `boxResolveOpts`/`withLocalRawRefs` moved to `spec.BoxResolveOpts`/`spec.WithLocalRawRefs` rather than being duplicated across the boundary. The earlier `hostBuildPrep` (`buildengine-prep`) leg is DELETED too: it existed only to populate the render-seam's Generator cache, which has no readers now that every render seam peer-dispatches. |

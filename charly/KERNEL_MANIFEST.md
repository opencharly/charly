...
| `check_endpoint_resolve.go` | ~115 (SHRUNK, #55 W3 B7) | M — wire-broker reverse-channel service surface ONLY (**genuine STAY**, settled by the B7 spike): the RESOLUTION BODIES relocated to `candy/plugin-check/resolve_endpoint.go` (compiled-in-REQUIRED placement class, `bed_session.go`'s precedent); this file keeps only the fixed reverse-RPC methods + the thin wire-forward calls to `verb:check-resolve`'s new `OpResolveEndpoint`/`OpResolveImageLabel`/`OpDrainEndpointCleanups` legs | `resolveVerbEndpoint`/`resolveImageLabel` are wired into `plugin_checkcontext_reverse.go`'s `checkContextReverseServer` via `provider_checkenv.go:187` (`resolveEp: h.resolveVerbEndpoint, resolveImgLabel: h.resolveImageLabel`) — the literal reverse-RPC service every out-of-process live-container verb (cdp/wl/vnc/dbus/mcp) dials back into; `hostVerbResolver.RunVerb` wraps `providerRegistry.ResolveVerb`, a core-private M-mechanism no out-of-process dispatch can bypass — the genuine reason this service surface itself cannot move. |
| `candy/plugin-check/resolve_endpoint.go` | ~150 (new, #55 W3 B7) | Compiled-in-REQUIRED placement class — `bed_session.go`'s SECOND member (`charly/host_build_check_bed_gpu_prereq.go`'s the first) | Holds the RESOLUTION WORK relocated from `check_endpoint_resolve.go`: reuses `venue.go`'s already-in-package `resolveCheckVenue`/`resolveCheckEndpoint` (built for the check-live gather engine, R3 — one resolver, not two) + `spec/container`'s image-label read. A live ssh-tunnel cleanup closure cannot cross ANY `Invoke` wire (compiled-in or not — `charly/provider.go`'s `Operation.Params`/`Result` are `json.RawMessage`, always bytes), so the plugin tracks its OWN per-process `pendingEndpointCleanups` state, drained only on the host's explicit `OpDrainEndpointCleanups` signal (`charly/check_endpoint_resolve.go`'s `runEndpointCleanups`) — the drain-after-Invoke LIFO ordering is pinned by `resolve_endpoint_test.go`. |
...
| `cue_kind_android_reg.go` + `cue_kind_box.go` + `cue_kind_candy.go` + `cue_kind_check.go` + `cue_kind_deploy.go` + `cue_kind_k8s.go` + `cue_kind_local.go` + `cue_kind_vm.go` + `cue_kind_pod.go` | 7+9+11+3+3+3+3+3+3 | D | each a `registerCueKind` data entry (3-11 LOC) wiring one kind word into the CUE kind registry `cue_schema.go` consults — pure D data, zero dispatch logic of their own. (audit-prod-report.md) |
...
| `loader_threaded.go` | 252 | D | registry-derived kind-recognition snapshot — the canonical D example the boundary-law skill itself cites. (audit-prod-report.md) |
...
| `config.go` | 78 | K1-IOU | `LoadConfig`/`LoadConfigRaw` entry points + `ResolveBox` wrappers — core-only per the K1 loader wave's own explicit "config.go/LoadUnified STAYS" verdict (cited by 12 live core call sites per the W2 `layers.go` row above). (audit-prod-report.md, corroborated by the existing K1-IOU convention on `layers.go`/`generate.go`/`format_config.go`) |
| `cue_defaults.go` | 25 | K1-IOU | `applyCueDefaults` — a CUE-unify (non-M, kind-blind) mechanism layered on the same K1 CUE-splice family as `validate.go`'s `coreCueSchema()`; not independently movable ahead of the K1 loader wave's own CUE-mechanism resolution. (audit-prod-report.md) |
| `cue_node.go` | 17 | K1-IOU | `validateNodeDocCUE` — own header: "The mechanism itself is `sdk/loaderkit.ValidateNodeDocCUE` (K1 unit 2); this file keeps a same-named/same-signature core wrapper (R3) since `GateDoc`/`materialize.go` call it by that exact function VALUE." A K1-loader-shaped thin wrapper, not independently movable. (own header, corroborated) |
...
| `main_repo.go` | 36 | K1-IOU | `ResolveProjectRepo` cache-path helper — feeds the K1-floor `refs.go`/`EnsureRepoDownloaded` git-clone/cache mechanism. (audit-prod-report.md) |
...
| `refs.go` | 84 | K1-IOU | own header: `EnsureRepoDownloaded`/`CollectRemoteRefs`/`CollectRemoteRefsOpts` bodies now live in `sdk/loaderkit` (K1 unit 4); this file keeps same-named/same-signature core wrappers (R3) building the `spec.RefsCollectSeams` host-coupled legs (registry-touching migrate-command dispatch, local-template substrate resolve, `CHARLY_REPO_OVERRIDE`) via `requireProjectLoader()` — the many production call sites keep working unchanged. Cross-referenced by `validate_project_host.go`'s already-documented K1-loader-refs-IOU row above (`validateRemoteCandies` calls `CollectRemoteRefs` here). Shrank 455->84 LOC since the audit as most of its former body already relocated in K1/W1. (own header, corroborated; audit-prod-report.md) |
| `refs_threaded.go` | 28 | K1-IOU | refs fetch-backend plugin wiring — feeds the same K1-floor refs.go mechanism above. (audit-prod-report.md) |
...
| `cue_schema.go` | 130 | MIXED: K1-IOU (CUE-unify splice mechanism, same family as validate.go's coreCueSchema()) / D (the per-kind registry cue_kind_*.go populates via init()) | own header: "One compiled schema instance..., a kind registry populated by each `cue_kind_<name>.go` via `init()`, and a per-entity validator" — matches the audit's own "R (mixed, ~30% D)" split; the CUE-splice mechanism half is the same K1 family as `validate.go`'s already-documented `coreCueSchema()` M row. Shrank 261->130 LOC since the audit. (own header; audit-prod-report.md) |
...
| `host_build_pod_lifecycle_dispatch.go` | 164 | R-RESIDUE (exit: W3a unit A10 — STAY+THIN, w3-adjudication.md item 4) | consolidated pod-* lifecycle CLI dispatch family — w3-adjudication.md item 4: "the hook registries + registry dispatch stay; the 8 near-identical handlers collapse to one op-discriminated handler... make it a unit, A10". Measured thin target ~80-140 LOC (from 164). (w3-adjudication.md item 4) |
| `host_build_render_service.go` | 30 | R-RESIDUE (exit: W3a A1-A8 deploy-dispatch cluster) | render-service HostBuild handler (capability). (audit-prod-report.md) |
...
| `pod_lifecycle_dispatch.go` | 205 | R-RESIDUE (exit: W3a unit A10 — STAY+THIN, w3-adjudication.md item 4) | pod start/stop opts threading + dispatch — the CORE half of the SAME A10 consolidation as `host_build_pod_lifecycle_dispatch.go` above ("205 + 164: STAY->STAY+THIN"). (w3-adjudication.md item 4) |

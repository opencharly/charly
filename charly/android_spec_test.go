...
func TestAndroidSpec_IsEndpoint(t *testing.T) {
	...
	ep := &AndroidSpec{Adb: &spec.AdbEndpoint{Host: "127.0.0.1:5037"}}
	if !ep.IsEndpoint() {
		t.Error("adb-source device should be an endpoint")
	}
	// Adb present but empty host = not an endpoint.
	if (&AndroidSpec{Adb: &spec.AdbEndpoint{}}).IsEndpoint() {
...
func TestApkPackageSpec_Defaults(t *testing.T) {
	s := spec.ApkPackageSpec{Package: "org.fdroid.fdroid"}
	...
	s2 := spec.ApkPackageSpec{Package: "x", Source: "f-droid", Arch: "arm64-v8a"}

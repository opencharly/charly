...
func (c *BuildCmd) runPrivilegedBootstrap(engine, dir, boxName string, img *buildkit.ResolvedBox) error {
	...
	ctx := struct {
		...
		Packages: buildkit.BootstrapPackagesForBox(img),
	}
	...
	if img.DistroDef != nil {
		ctx.ExtraPacmanConf = buildkit.RenderPacstrapExtraConf(img.DistroDef.Pacstrap)
		runtimeConf, rerr := buildkit.RenderRuntimePacmanConf(img.DistroDef.Pacstrap)
		if rerr != nil {
			return rerr
		}

...
//
// The selection SHAPES share the ONE OpCompile request (see #DeployCompileRequest's doc
// comment, sdk/schema/seam.cue, for the authoritative discrimination). Every shape's SELECTION now
// resolves plugin-side off the resolved-project envelope; the host sends only ref names + the
// host-computed HostContext:
...
//     sends only box_ref.
//   - ADD-CANDY-ON-BOX (compileAddCandyOnBox, the add_candy-on-pod/k8s shape, K4 box-half
//     completion) — the plugin reads rp.Boxes[base_box_ref] as the compile context and resolves
//     the overlay candy_ref's own order off the envelope; the host sends only candy_ref +
//     base_box_ref. Replaces the former host-side ResolveBox(baseImg) + scanCandiesForRef.
//   - BOX-VIEW (a base image projected host-side, sending box_view+order) — retained on the
//     #DeployCompileRequest for any caller still resolving a base image host-side; no host entry
//     point in this file builds it any more.
//
// R5: the old compilePlans/compileBoxPlans/compileCandyPlans/compileCandyPlansWithContext loop
// bodies, plus EVERY shape's host-side selection computation (ResolveBox, ScanAllCandyWithConfig,
// deploykit.ResolveCandyOrder, the synthetic box constructors, the builder pre-connect) are ALL
// deleted — see each compile* helper's own doc comment for why the builder pre-connect
// specifically is provably redundant (candy/plugin-bundle's preresolveBuilderContexts already
// S2-lazy-connects unconditionally), not merely relocated.
...
// whole-deploy (alongside the plugin-side builder pre-pass), so
...
// machine venue is systemd in practice, per deploykit.PruneContainerInitForSystemd's pre-existing
...
// plugins the former host-side builder pre-connect handled (verified live, K4 unit
...
// the former host-side builder pre-connect handled — unconditionally, for every
...
// compileAddCandyOnBox is the K4-box-half-completion ADD-CANDY-ON-BOX shape: the add_candy overlay
// `alRef` compiled against the primary pod/k8s base image `baseBoxRef`, ALL resolved plugin-side off
// the resolved-project envelope (candy/plugin-bundle's resolveAddCandyOnBoxSelection). Replaces the
// former host-side buildkit.ResolveBox(baseImg) + scanCandiesForRef + host builder pre-pass: the
// base-box read reuses the BOX-REF shape's already-proven envelope parity, the overlay-order read
// reuses the CANDY shape's, and the builder pre-pass runs unconditionally inside the plugin's
// compileDeployPlans (preresolveBuilderContexts) — so nothing is re-derived host-side. hostCtx is
// still host-computed (MachineVenue probe + the LoadUnified-coupled active-init lookup), same as
// every other shape. ExtraCandyRefs carries alRef.Raw so the plugin's OWN resolved-project re-fetch
// discovers a REMOTE overlay ref (RCA'd K1-alpha regression: check-addcandy-pod/check-stepkind-emit-pod,
// "candy not in resolved-project envelope").
func (c *deployAddCmd) compileAddCandyOnBox(alRef *DeployRef, baseBoxRef, dir string) ([]*deploykit.InstallPlan, error) {
	hostCtx := c.compileHostContext()
	hostCtx, err := preresolveActiveInitInto(hostCtx, dir)
	if err != nil {
		return nil, err

...
func TestRenderCacheMountsAuto_Mixed(t *testing.T) {
	mounts := []spec.CacheMount{
		...
		{Dst: "/var/cache/charly/aur-srcdest", Owned: true},   // user build cache
		{Dst: "/var/cache/charly/aur-xdg-cache", Owned: true}, // user build cache
	}
	...
	if !strings.Contains(out, "id=charly-var-cache-charly-aur-srcdest-uid1000,dst=/var/cache/charly/aur-srcdest,uid=1000,gid=1000") {
		t.Errorf("srcdest entry must be uid-owned:\n%s", out)
	}
	if !strings.Contains(out, "id=charly-var-cache-charly-aur-xdg-cache-uid1000,dst=/var/cache/charly/aur-xdg-cache,uid=1000,gid=1000") {
...
func TestRenderCacheMounts_TrailingSeparator(t *testing.T) {
	mounts := []spec.CacheMount{{Dst: "/var/cache/charly/pixi-cache"}}
	got := spec.RenderCacheMounts(mounts, 1000, 1000, " \\\n    ", true)
	if !strings.HasSuffix(got, " \\\n    ") {
		t.Errorf("trailing separator missing; got: %q", got)
	}
	if !strings.Contains(got, "id=charly-var-cache-charly-pixi-cache-uid1000") {

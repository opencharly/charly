...
// deploykit.SaveDeployConfig/PersistBedDeployOverrides directly either moved to
...
// testLoadDeployConfig (the READ half) survives — it is charly's own LoadUnified, the genuine
// charly-loader concern these seam-driven tests verify against, now built on the local
// testProjectDeployConfig port (plugin_sidecar_test.go) instead of sdk/deploykit.ProjectDeployConfig
...
// testLoadDeployConfig reads the per-host overlay through charly's REAL LoadUnified — the
...
func testLoadDeployConfig() (*spec.DeployConfig, error) {
	...
	return testProjectDeployConfig(uf), nil
}

...
// only fail under concrete validation; the seam's ValidateEntityClosedCUE alone would silently
...
func validateKindBody(t *testing.T, kind, yamlBody string) error {
	t.Helper()
	doc, err := requireProjectLoader().CueDocFromYAML("t.yml", []byte(yamlBody))
	if err != nil {
		return err
	}
	return requireProjectLoader().ValidateEntityCUE(kind, "t.yml:"+kind, doc)
}
...
func TestCueTightening_RejectsAndAccepts(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			var err error
			if tc.kind == "candy" {
				err = requireProjectLoader().ValidateCandyManifestCUE("t.yml", []byte(tc.yaml), loaderThreaded(), requireLoaderParser())
			} else {

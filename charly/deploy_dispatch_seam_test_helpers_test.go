...
// (dispatchDeployTarget → the compiled-in command:deploy plugin's Invoke(OpDeployDispatch) →
// handleDeployApply/handleDeployDel → lifecycleInvoke → the plugin's own deployMarshalNode +
// loadDeployConfig + deploykit writer) instead of calling the sdk writer directly — the

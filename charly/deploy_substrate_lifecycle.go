...
type substrateLifecycle interface {
	...
	PrepareVenue(ctx context.Context, name, dir string, node *spec.BundleNode, plans []*InstallPlan, opts EmitOpts) (DeployExecutor, error)
	...
	PostApply(ctx context.Context, name, dir string, node *spec.BundleNode, exec DeployExecutor, opts EmitOpts) error
	...
	VenueExecutor(name string, node *spec.BundleNode) (DeployExecutor, error)

...
// command:deploy's OpDeployDispatch (the ONE generic envelope every UnifiedDeployTarget/
...
// command:deploy is COMPILED-IN (an inprocProvider). The plugin's own OpDeployDispatch handler
...
func dispatchDeployTarget(ctx context.Context, req spec.DeployTargetDispatchRequest, exec spec.DeployExecutor, build buildEngineContext, rebootable bool) (spec.DeployTargetDispatchReply, error) {
	...
	if !ok {
		return spec.DeployTargetDispatchReply{}, fmt.Errorf("deploy-dispatch %s: command:deploy provider not loaded (candy/plugin-fleet must be compiled in via compiled_plugins:)", req.Op)
	}
	...
	res, err := prov.Invoke(invokeCtx, &Operation{Reserved: "deploy", Op: ops.OpDeployDispatch, Params: reqJSON})
	if err != nil {
		return spec.DeployTargetDispatchReply{}, fmt.Errorf("deploy-dispatch %s: deploy plugin: %w", req.Op, err)
	}

...
// keystone): the per-node TERMINAL step of `charly deploy add`. The plugin ships ALREADY-COMPILED
...
func runResolveTargetAdd(req spec.DeployResolveTargetAddRequest) error {
	...
	if req.Node == nil {
		req.Node = &spec.DeployNode{Target: req.Target} // ref-based deploy: synthesize from the plugin-classified target
	}

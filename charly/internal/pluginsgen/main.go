...
func run(root, cfg, outGo, outWork string) error {
	// Bootstrap guard: the generated go.work references the sdk submodule; a fresh
	// non-recursive clone has an EMPTY sdk/, which would fail much later with an opaque
	// workspace error. Fail here, first, with the remediation. (The spec contract
	// module needs no checkout: it resolves from the module proxy at the pinned
	// require version in every go.mod.)
...
func generate(root, cfg string) (genGo, genWork []byte, err error) {
	...
	w.WriteString("// The charly module, the sdk contract submodule, plus every plugin\n")
	...
	w.WriteString("// charly/plugins_generated.go through these `use` directives. The spec\n")
	w.WriteString("// contract module (github.com/opencharly/spec) is NOT a workspace member:\n")
	w.WriteString("// every module requires it at one pinned version, resolved from the proxy.\n")

...
func generate(root, cfg string) (genGo, genWork []byte, err error) {
	...
	w.WriteString("// charly.yml `compiled_plugins:`. `go build ./charly` resolves the\n")
	w.WriteString("// compiled-in candy imports in charly/plugins_generated.go through these\n")
	w.WriteString("// `use` directives. The contract modules (github.com/opencharly/sdk and\n")
	w.WriteString("// github.com/opencharly/spec) are NOT workspace members: every module\n")
	w.WriteString("// requires them at one pinned version, resolved from the proxy.\n")

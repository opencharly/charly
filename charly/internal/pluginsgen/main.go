...
// charly and without the candy modules resolving — which is what lets `task build:binary`
...
func generate(root, cfg string) (genGo, genWork []byte, err error) {
	...
	for _, name := range names {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("compiled plugin %q: fetch %s@%s: %w", name, repoPath, version, err)
		}
	...
	g.WriteString("// `compiled_plugins:`. Regenerated by `task build:binary` before `go build`; the\n")

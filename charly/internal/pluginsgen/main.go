...
func run(root, cfg, outGo, outWork string) error {
	// Bootstrap guard: the generated go.work references the sdk submodule plus the
	// in-repo spec module; a fresh non-recursive clone has an EMPTY sdk/, which would
	// fail much later with an opaque workspace error. Fail here, first, with the
	// remediation. (spec is vendored into the main repo — spec/go.mod is a tracked
	// file — so only sdk can be missing on a fresh clone.)
	if _, err := os.Stat(filepath.Join(root, "sdk", "go.mod")); err != nil {
		return fmt.Errorf("sdk/go.mod missing — the sdk submodule is not initialized; run: git submodule update --init sdk")
	}
	if _, err := os.Stat(filepath.Join(root, "spec", "go.mod")); err != nil {
		return fmt.Errorf("spec/go.mod missing — the spec module tree is corrupted or incomplete; restore spec/ from the repository")
	}
...
func generate(root, cfg string) (genGo, genWork []byte, err error) {
	...
	w.WriteString("// The charly module, the sdk contract submodule, the in-repo spec module, plus every plugin\n")
	w.WriteString("// candy selected into the binary via charly.yml `compiled_plugins:`. `go build\n")
	w.WriteString("// ./charly` resolves the compiled-in candy imports in\n")
	w.WriteString("// charly/plugins_generated.go through these `use` directives.\n")
	w.WriteString("use ./charly\n")
	w.WriteString("use ./sdk\n")
	w.WriteString("use ./spec\n")
	for _, e := range entries {

...
func run(root, cfg, outGo, outWork string) error {
	// Bootstrap guard: the generated go.work references the sdk + spec submodules;
	// a fresh non-recursive clone has an EMPTY sdk/ or spec/, which would fail much
	// later with an opaque workspace error. Fail here, first, with the remediation.
	...
	if _, err := os.Stat(filepath.Join(root, "spec", "go.mod")); err != nil {
		return fmt.Errorf("spec/go.mod missing — the spec submodule is not initialized; run: git submodule update --init spec")
	}
...
func generate(root, cfg string) (genGo, genWork []byte, err error) {
	...
	w.WriteString("// The charly module, the sdk + spec contract submodules, plus every plugin\n")

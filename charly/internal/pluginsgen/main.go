...
func run(root, cfg, outGo, outWork string) error {
	// Bootstrap guard: the generated go.work references the sdk submodule; a
	// fresh non-recursive clone has an EMPTY sdk/, which would fail much later
	// with an opaque workspace error. Fail here, first, with the remediation.
...
func generate(root, cfg string) (genGo, genWork []byte, err error) {
	...
	w.WriteString("// The charly module, the sdk contract submodule, plus every plugin candy\n")
	w.WriteString("// selected into the binary via charly.yml `compiled_plugins:`. `go build\n")

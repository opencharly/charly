...
var kernelFloor = []floorEntry{
	...
	{"check_bed_run.go", "M — host-side bed-helper legs the plugin-check bed-runner reaches over HostBuild + the check-bed session seam: bedExternalInPlace (the ONE registry-coupled substrate classification — queries the live provider registry, threaded as CheckBedReply.IsExternal), bedCheckLevel (resolves the box ref against the loaded *UnifiedFile, core-only). The bed-root + member deploy-override persist moved PLUGIN-SIDE to candy/plugin-check (#55 coneC-dsh β1 — the former persistBedDeployOverrides wrapper + its deploykit import shed); the orchestration moved to deploykit (unit 6b); a marshalled plugin can't compute these host-coupled legs"},
	{"check_cmd.go", "M — residual host-side check-plumbing serving the check-load-plugins seam (resolveCheckRunnerContext: host plugin-load side effect + committed-APK anchoring) + the `target: local` deploy `--verify` path (checkLocalDeployScope, kind-blind — op.Kind is the step discriminator); the live/feature-live arms already moved to plugin-check. Uses the ONE canonical kind-blind address-parser spec.SplitVmAddress (strips the legacy `vm:` CLI-address prefix in resolveDeployNodeByPath, R3 — the SAME parser resolveDelNode/bundle_add use; relocated to spec/spec/vm_domain.go in #55 vmshared Bucket B, so check_cmd.go imports spec, not vmshared)"},
	...
	{"config.go", "M — the thin host LoadConfig/LoadConfigRaw entry seams over the LoadUnified spec.ProjectLoader seam + uf.ProjectConfig, plus resolveVocabOpts (the host projection loading the project vocabulary into spec.ResolveOpts). The load ORCHESTRATION moved to loaderkit (#48) and is reached through the plugin-served ProjectLoader seam (#55 keystone); this is the thin host loader entry"},
	...
	{"deploy.go", "D — the DeployConfigPath/DeployConfigEnv charly.yml-path seam pointers (bodies in sdk/kit; ONE definition shared with candy/plugin-migrate). The deploy-key→box loader-read resolvers that once lived here collapsed into candy/plugin-deploy-pod (resolve_ref.go) with the pod-config-resolve-ref reverse leg — #55 Cone A Unit 2 — so deploy.go no longer imports sdk/deploykit or sdk/spec; it is now purely the config-path data seam"},
	...
	{"k8s_config.go", "M — the host-side k8s loader-read HELPER of the FLOORED deploy-entity-resolve seam (findK8sSpec's SOLE caller is host_build_deploy_entity_resolve.go, in kernelFloor): LoadUnified→spec.UnifiedFile.ProjectTemplates.K8s→resolveK8sViaPlugin, the kind:k8s→ResolvedK8s resolution the generic seam performs for plugin-kube. Same floor-class as its seam; NOT a raw-project move (that would duplicate the seam's resolution + entangle substrate_template_resolve)"},
	...
	{"raw_project_host.go", "M — the generic \"raw-project\" HostBuild seam: the CHEAP kind-blind host-resident loader-read broker-seam half serving ANY consumer (spec.UnifiedFile.ProjectTemplates + the folded Descent-stamped uf.Bundle + loaderThreaded().Primaries, no ResolveBox) — the endgame keystone the loader-coupled deploy plugins reach, same class as resolved_project_host.go/host_build_cli"},
	{"readiness_config.go", "M — the host ReadinessProvider wiring: loadedReadiness resolves the project defaults.readiness once via LoadUnified (host-resident) + sets spec.ReadinessProvider at init; readinessPluginEnv emits the CHARLY_READINESS_* plugin-spawn env from it. Consumed by the FLOORED host_build_deploy_artifacts_retrieve seam + the plugin transport spawn (plugins carry their OWN loadedReadiness copies — plugin-vm/plugin-check). Generic host loader-read + transport-init wiring, kind-blind"},
	...
	{"refs_threaded.go", "D/M — the registered remote-repo FETCH BACKEND hook (activeRefsDownloader, the spec.RefsDownloader of the compiled-in candy/plugin-refs, swappable git→OCI/S3); NO in-core fallback (requireRefsDownloader FATALs if plugin-refs was not compiled in), mirrors activeLoaderParser (loader_threaded.go). A host D-hook the EnsureRepoDownloaded seam dispatches through"},
	...
	{"update_deploy_dispatch.go", "M — the kind-blind `charly update` deploy-target dispatch kernel: dispatchByDeployTarget (consumes the plugin-threaded merged tree + loadDeployPlugins + ResolveTarget.Rebuild — dispatches by substrate word via the registry, NO per-kind branch) consumed by the FLOORED host_build_pod_lifecycle_dispatch seam; + resolveUpdateDeployNode (pure spec.DeployKey lookup) + noteUpdateDisposability (pure transparency print). The deploy-walk dispatch M, sibling of unified_targets.go's ResolveTarget"},
	...
	{"host_build_config_resolve.go", "M — the config-resolve F10 host-builder (loader/registry reverse-leg the plugin calls back; kind-blind). #55 coneC: the kit.ResolveRuntime host-runtime probe shed — now spec/hostenv.ResolveRuntime, no sdk/kit import (kit.ResolvedRuntime was a type alias = hostenv.ResolvedRuntime, so the ResolveRuntime call is the sole kit use)"},
	...
	{"host_build_pod_config_seams.go", "M — the pod-config F10 host-builders the plugin calls back (loader/registry/credential-coupled reverse-legs). #55 coneC: the kit container-runtime family shed — ResolveRuntime/ResolvedRuntime→spec/hostenv, LocalImageExists/TransferImage/ContainerImage→spec/container, ErrImageNotLocal/ContainerNameInstance→spec/spec (all were already spec re-exports; TransferImage's body relocated to spec/container/transfer.go, kit re-exports it). No sdk/kit import remains"},
	...
	{"checkrun.go", "E — the package-main RunMode binding onto spec.CheckRunMode (RunModeLive/RunModeBox) + the check-verdict result constructors (passf/failf/skipf over spec.CheckResult); spec-only glue, ZERO kit import. The former in-proc newCheckRunner/carrierFromRunner drive moved PLUGIN-SIDE (command:check OpVerifyChecks, #55 CHECK-ENGINE cone Unit 2 — candy/plugin-check builds the runner now); its charly-side tests drive the production OpVerifyChecks seam instead (dispatchCheckOpsMode, checkrun_helpers_test.go)"},
	...
	{"host_build_box_ref_resolve.go", "M — resolveImageRefForEnsure ONLY: the host-internal box-ref resolve helper for plugin_executor_reverse.go's BuilderStep ResolveImage closure (deploykit.ResolveSpecBox; the \"box-ref-resolve\" HostBuilder + buildableShortNameForEnsure are DELETED — #55 coneK1 #8: build:ensure resolves the box ref plugin-side via the K1 loader reverse legs; the deploykit-count drop for this file is coneD-gated on moving the caller plugin-side)"},
	...
	{"host_build_remote_image_resolve.go", "M — the remote-image-resolve seam for plugin-build's ensure fallback AND candy/plugin-box's `box build @ref` (K1 loader wave: clone/cache ONLY via EnsureRepoDownloaded; the box-RESOLVE + registry pull ref moved plugin-side — plugin-build loads the cached cfg via the K1 loader reverse legs + reads the box registry/name itself, shedding deploykit from charly core; remote_image.go DELETED, deploykit 15→14)"},
	...
	// plugin-deploy-pod).
	{"service_render.go", "M — RenderService: thin providerRegistry.ResolveKind(\"init\") dispatch, the direct callee of the already-floor host_build_render_service.go; also carries the egress-validation dispatch merged from the deleted charly/egress.go (coneB-buildtail) — thin verb:egress registry-dispatch plus the load-bearing spec.ValidateRecord init-seam wiring the sdk libraries' function-var injection point requires (they cannot import charly core; the former sdk/vmshared egress fills were deleted in #55 vmshared Bucket D — their consumers run only in candy/plugin-vm, which fills them itself)"},
	...
	{"host_build_buildengine.go", "M — the shrunk render-seam-floor consumer (hostBuildPrep/hostBuildContextIgnoreBaseline) + the 6 other buildengine-* K1-loader-witness legs (bootstrap-delicate local scan, git clone/cache, build-time plugin CONNECT registry-M, namespaced-box flat NamespaceScanReply recursion: projectCandiesScanned + the namespace-scoped reachability walk) resolveBuildEngine reaches for genuinely host-only steps — the deploykit render-prep moved plugin-side (foldNamespaceScanEntries); mirrors host_build_loader_floor.go's loader-* legs, already floor"},
	...
	{"generate.go", "M — the shared render-seam-floor Generator TYPE (Boxes now the wire-clean *spec.ResolvedBox, #55 Cluster-B) + its ONE remaining cheap constructor (newCandyScanGenerator), which stores the CALLER-PUSHED box set (no deploykit import — #55 coneB2 Class B: the box RESOLVE shed from the host; candy/plugin-build resolves via buildkit.ResolveAllBox + deploykit.SpecBoxes and pushes on #ResolvedProjectRequest.boxes, charly names no buildkit/deploykit type). newCandyScanGenerator is consumed ONLY by the already-floor F10 host-builders (host_build_buildengine.go, host_build_render_seam.go's loadRenderGen fallback) + render_seam_cache_test.go. Also carries the shared OpResolve builder-stage dispatch (resolveBuilderStage, over *spec.ResolvedBox) — thin providerRegistry.ResolveBuilder-coupled M dispatch, the same shape as the other floor OpResolve seams above; consumed by tasks.go's resolveInlineBuilderSeam. baselineContextIgnore additionally has a hard structural floor reason: it //go:embeds charly/charly.yml, only possible from this module"},
	{"tasks.go", "M — the ONE surviving render-seam builder impl (resolveInlineBuilderSeam: providerRegistry.ResolveBuilder + ensureBuildersConnected + the shared resolveBuilderStage OpResolve dispatch, live via host_build_render_seam.go's RenderSeamInlineBuilder case) + invokeOpEmitFragmentOpt (the pod-overlay external-STEP OpEmit → EmitReply.Fragment splice, ociEmitStep): thin providerRegistry-coupled M seam-wiring bridging deploykit's render engine to the core provider registry, zero capability logic. The former host-side toDeploykit()/dkGen/invokeVerbBuildEmit/ResolveDetection+ExternalBuilderStageSeam EmitPluginOp bridge was production-DEAD (the live plugin-verb build-emit runs plugin-side in candy/plugin-build via InvokeProvider(OpEmit)) and DELETED in #55 cone-render Unit A"},
	...
	{"build_overlay.go", "M — hostBuildOverlay is the F10 pod-overlay PREP host-builder (registered like the ~25 other host_build_*.go floor entries), SHRUNK FURTHER in #55 coneB-br2: the secret resolution (resolveCandySecrets + deploykit.InjectSecretsIntoPlans) + the base-distro resolve (resolveOverlayBaseDistroDef → deploykit.ResolveSpecBox for build.DistroCfg) MOVED PLUGIN-SIDE to candy/plugin-deploy-pod's buildOverlay (mirroring candy/plugin-bundle/secrets_artifacts.go: SelectCandiesForPlans + ResolveSecretForCandy over CredentialAccessViaExecutor; the base-distro render is plugin-side via dg.Boxes[env.Image].DistroDef in candy/plugin-installstep's emitSystemPackages) — enc.go + layer_secrets.go DELETED, this file's sdk/deploykit import is GONE. What remains is genuinely host-only: base-image metadata a live podman executor must read (BaseUser/BaseSecurity/BaseRegistry via ExtractMetadata) + re-attaching the live plans + parent venue from ctx — plus a minimal buildEngineContext (bare Box/Generator literals, DistroCfg nil — dead on the step-emit path) for the \"oci-emit-step\" step-emitter, shared with the already-floor host_build_render_seam.go's renderGenCache/overlayBuildContextCache — zero capability RENDER or RESOLVE logic left, imports spec/* only"},
	...
	{"bundle_add_cmd.go", "M — the host-side residue of `charly bundle add`/`charly bundle del`: deriveChildExecutorForPath already carries its own E/M/D-VERIFIED doc comment (registry-coupled deployTraitDescent, constructs a live DeployExecutor that can't cross the wire); deployDelCmd/resolveDelNode/podDeploymentArtifactExists reconstruct the del-resolve seam's target node from a wire request (registry + deploy-tree + container-store probes); loadConfigForDeploy is host-fs + already-floor LoadConfig/LoadDefaultBuildConfig/RegisterBuildVocabulary glue shared with the already-floor build_overlay.go (detectHostContext + resolveDistroDef were DELETED in #55 coneB-br2 Group 2 — deadened by the resolveOverlayBaseDistroDef drop; the deploy compile uses the plugin's own twin candy/plugin-bundle/dispatch.go detectHostContext)"},

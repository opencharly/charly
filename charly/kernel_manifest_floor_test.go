...
var kernelFloor = []floorEntry{
	...
	{"generate.go", "M — the shared render-seam-floor Generator TYPE + its ONE remaining cheap constructor (newCandyScanGenerator) — the expensive NewGenerator (full resolve: ComputeIntermediates/GlobalCandyOrder/ComputeEffectiveVersions) is DELETED (#55 step3 3-II): its last production caller, build_overlay.go's hostBuildOverlay, now reaches the SAME full-resolve recipe plugin-side via candy/plugin-build's resolveBuildEngine instead — no host-side full resolve remains. newCandyScanGenerator is consumed ONLY by the already-floor F10 host-builders (host_build_buildengine.go, host_build_render_seam.go's loadRenderGen fallback) — call-graph verified, no caller exists outside those 2 + 2 test-only reproductions (render_seam_cache_test.go, plugin_installstep_envelope_parity_test.go's testFullResolveGenerator, which inlines the deleted NewGenerator's recipe test-side). Also carries the registry-coupled verb:oci adopt-user dispatch (ociProvider/invokeOciInspectUser) and the shared OpResolve builder-stage dispatch (resolveBuilderStage/resolveExternalBuilder) — thin providerRegistry.ResolveBuilder/resolve(ClassVerb)-coupled M dispatch, the same shape as the other floor OpResolve seams above. baselineContextIgnore additionally has a hard structural floor reason: it //go:embeds charly/charly.yml, only possible from this module"},

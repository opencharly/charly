...
var kernelFloor = []floorEntry{
	...
	{"generate.go", "M — the shared render-seam-floor Generator TYPE (Boxes now the wire-clean *spec.ResolvedBox, #55 Cluster-B) + its ONE remaining cheap constructor (newCandyScanGenerator), which resolves boxes via the deploykit box-resolve bridge (deploykit.ResolveAllSpecBoxes — the PURE buildkit resolve run kit-side, charly names no buildkit type). newCandyScanGenerator is consumed ONLY by the already-floor F10 host-builders (host_build_buildengine.go, host_build_render_seam.go's loadRenderGen fallback) + render_seam_cache_test.go. Also carries the shared OpResolve builder-stage dispatch (resolveBuilderStage/resolveExternalBuilder, now over *spec.ResolvedBox) — thin providerRegistry.ResolveBuilder/resolve(ClassVerb)-coupled M dispatch, the same shape as the other floor OpResolve seams above. baselineContextIgnore additionally has a hard structural floor reason: it //go:embeds charly/charly.yml, only possible from this module"},
	{"tasks.go", "M — Generator.toDeploykit() (feeds its *spec.ResolvedBox Boxes to the deploykit render engine via deploykit.WrapSpecBoxes, #55 Cluster-B) + the 3 ResolveXStageSeam closures + EmitPluginOp: 100% providerRegistry-coupled seam-wiring bridging deploykit's render engine to the core provider registry (EnsureBuildersConnected/ResolveDetectionBuilderStage/ResolveExternalBuilderStage/ResolveInlineBuilder — all now over *spec.ResolvedBox) — zero movable render logic remains, the render DRIVE itself already moved to deploykit in prior cutovers (K3-A/K5-Unit-6b); every function call-graph-verified referenced, no dead code"},

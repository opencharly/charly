...
var kernelFloor = []floorEntry{
	...
	// plugin-deploy-pod).
	{"service_render.go", "M — RenderService: thin providerRegistry.ResolveKind(\"init\") dispatch, the direct callee of the already-floor host_build_render_service.go; also carries the egress-validation dispatch merged from the deleted charly/egress.go (coneB-buildtail) — thin verb:egress registry-dispatch plus the load-bearing kit.ValidateRecord init-seam wiring sdk/kit's function-var injection point requires (it cannot import charly core; the former sdk/vmshared egress fills were deleted in #55 vmshared Bucket D — their consumers run only in candy/plugin-vm, which fills them itself)"},

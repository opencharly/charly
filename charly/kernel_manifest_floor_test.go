...
var kernelFloor = []floorEntry{
	...
	{"layer_secrets.go", "M — CandyForPlan (ScanAllCandyWithConfig + *Config, both core-only loader mechanics no plugin can run itself) applying the shared deploykit.SelectCandiesForPlans pick, + resolveCandySecrets (its thin deploykit.ResolveSecretForCandy wrapper feeding coreCredentialAccess). SOLE caller is build_overlay.go's hostBuildOverlay (the pod-overlay build, already host-side): #55 K4 deleted the deploy-candy-secrets/deploy-artifacts-retrieve seams — command:bundle's deploy-add path now resolves candy secrets + retrieves artifacts PLUGIN-SIDE (candy/plugin-bundle) over the envelope candy set + verb:credential + its live venue"},
	...
	{"build_overlay.go", "M — hostBuildOverlay is the F10 pod-overlay PREP+RESOLVE host-builder (registered like the ~25 other host_build_*.go floor entries), SHRUNK FURTHER in #55 step3 3-II: candy/plugin-deploy-pod's podPrepareVenue now fetches its OWN render-prepped resolved-project envelope via InvokeProvider(\"build\",\"generate\",OpResolve,…) instead of this seam projecting it (projectResolvedProjectWithBoxes/loadProjectForResolve/NewGenerator/createRemoteCandyCopies/the per-candy-security loop/init-system resolution are ALL GONE from this file). What remains is genuinely host-only: base-image metadata a live podman executor must read (BaseUser/BaseSecurity/BaseRegistry via ExtractMetadata), resolveCandySecrets, and re-attaching the live plans + parent venue from ctx — plus a minimal buildEngineContext (bare Box/Generator literals, no full resolve) for the \"oci-emit-step\" step-emitter, shared with the already-floor host_build_render_seam.go's renderGenCache/overlayBuildContextCache — zero capability RENDER or RESOLVE logic left"},

...
// host folds the ParsedProject back into the typed *UnifiedFile. parsedNodeToGeneric reconstructs
...
// preserving the pre-K1 call shape (*genericNode, *UnifiedFile) for tests that build a genericNode
...
// document's decomposed nodes) into the typed UnifiedFile, node by node via materializeNodeInto.

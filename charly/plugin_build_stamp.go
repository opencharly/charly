...
// dependency graph (the sdk submodule and the in-repo spec module, reached through the candy's
// `replace` directives), so skipping a rebuild on a path match would hand back a STALE binary
// after a sdk submodule bump or an in-repo spec edit. That
// reasoning is right, and this stamp does not weaken it: it keys freshness on CONTENT, so a
// submodule bump — or any uncommitted edit inside either tree — changes the stamp and forces the rebuild.
//
...
// catch a submodule bump, but it is WRONG here: the sdk submodule and the in-repo spec tree carry
// uncommitted working-tree edits for most of a development session, and a HEAD-keyed stamp would
// happily reuse a binary built before those edits — reintroducing exactly the staleness #76 warned
// about, in the case that matters most day to day. Content hashing catches both.
...
// candy replaces sdk, and sdk replaces spec, an in-repo module).
//

...
// verbs (charly check, charly deploy add, …) drive it invisibly.
...
// server the ExecutorTransport reattaches to. transport.Serve writes the handshake to
// stdout and blocks serving; the host reaps it via client.Kill (the gRPC Shutdown
// RPC), and transport.Serve's parent-death watch self-terminates this process if the
...
func (c *PluginServeCmd) Run() error { //nolint:unparam // Kong Run signature requires error; transport.Serve blocks until disconnect
	...
	transport.Serve(&providerGRPCServer{set: set}, &metaGRPCServer{set: set})
	return nil
}

...
// and is consumed by host_build_construct_step.go and provider_checkenv.go
// (host_build_pod_config.go was deleted in K-wave 2 cone R3).
...
func (s *executorReverseServer) InvokeProvider(ctx context.Context, req *pb.InvokeProviderRequest) (*pb.InvokeReply, error) {
	...
	default:
		if inv, isInv := prov.(executorInvoker); isInv {
			...
			// a manual specexec.ContextWithExecutor (status-substrate / build / command dispatch). Every

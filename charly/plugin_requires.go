...
// plugin.requires entry: Class+Word name the peer ("<class>:<word>"), Source is the
// peer's candy ref for a peer outside the project closure, Optional makes an absent
// peer a recorded skip rather than a load failure.
...
// key renders the peer as a registry key ("<class>:<word>") for messages + lookup.
func (r Requirement) key() string { return r.Class + ":" + r.Word }
...
func liftRequirements(required []*pb.PluginRequirement, origin string) ([]Requirement, error) {
	...
	for _, r := range required {
		...
		out = append(out, Requirement{Class: r.GetClass(), Word: r.GetWord(), Source: r.GetSource(), Optional: r.GetOptional()})
	}
	return out, nil
}
...
func registerPluginUnitRequires(unitName string, unit *PluginUnit) error {
	...
	for _, req := range unit.Requires {
		...
		if _, ok := providerRegistry.resolve(class, req.Word); ok {
			continue
		}
		if _, ok := connectPluginByWordRef(class, req.Word, req.Source); ok {
			continue
		}
		if req.Optional {
...
// requiresErrorHint keeps the message symmetry helper out of the hot path; removed.

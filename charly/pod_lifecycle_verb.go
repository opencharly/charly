...
// the cycle-free plugin-side overlay read — the dc.Deploy[key] lookup + the container/""→pod Target
...
// external substrate plugins first, R3 with deploy add / update), returning a clear error for a
...
func dispatchLifecycleTarget(verb string, node *spec.DeployNode, deployName string) (spec.LifecycleTarget, error) {
	if node == nil {

...
// (externalizedDeploySubstrates) — ALL SIX substrates externalize today; there is no
...
// it — so the two gates can never disagree. GENERAL for all 6 — ALL SIX substrates
...
// type-assertion branch in unified_targets.go were confirmed dead (zero implementers)
// and deleted; ALL SIX substrates externalize today. Run in the same init() that

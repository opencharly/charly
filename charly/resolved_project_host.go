...
func projectResolvedProject(cfg *Config, layers map[string]spec.CandyReader, uf *loaderkit.UnifiedFile, distroCfg *buildkit.DistroConfig, builderCfg *buildkit.BuilderConfig, initCfg *InitConfig, dir, version string, opts ResolveOpts, diags *spec.Diagnostics) (*spec.ResolvedProject, error) {
...
func projectResolvedProjectWithBoxes(cfg *Config, layers map[string]spec.CandyReader, uf *loaderkit.UnifiedFile, distroCfg *buildkit.DistroConfig, builderCfg *buildkit.BuilderConfig, initCfg *InitConfig, dir, version string, opts ResolveOpts, diags *spec.Diagnostics, preResolvedBoxes map[string]*buildkit.ResolvedBox) (*spec.ResolvedProject, error) {
	...
	if uf != nil {
		fillNamespacedBoxes(uf, initCfg, "", calver, dir, opts, rp, map[*loaderkit.UnifiedFile]bool{})
	}
	...
	if uf != nil {
		if resources := resolveResources(uf); len(resources) > 0 {
...
func fillNamespacedBoxes(uf *loaderkit.UnifiedFile, initCfg *InitConfig, prefix, calver, dir string, opts ResolveOpts, rp *spec.ResolvedProject, visited map[*loaderkit.UnifiedFile]bool) {
...
func projectTemplates(uf *loaderkit.UnifiedFile) *spec.ProjectTemplates {
	t := &spec.ProjectTemplates{}
	fillNamespacedTemplates(uf, "", t, map[*loaderkit.UnifiedFile]bool{})
...
func fillNamespacedTemplates(uf *loaderkit.UnifiedFile, prefix string, t *spec.ProjectTemplates, visited map[*loaderkit.UnifiedFile]bool) {
	if uf == nil || visited[uf] {
		return
	}
	visited[uf] = true

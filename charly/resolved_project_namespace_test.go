...
// A3-phase-2's Resolve{Kubernetes,Vm,Android}EntityViaExecutor — LoadUnifiedViaExecutor's own doc
...
// TestHostBuildDeployEntityResolve_KubernetesNamespaceQualified functional-proof test are BOTH deleted
...
// kind:kubernetes cluster profile (prod-cluster) — mirroring the real check-k3s-vm-ctx shape in this
// repo's own charly.yml (`kubernetes: {box: "", kubeconfig_context: ...}`).
...
// kind:kubernetes entity into the envelope under its QUALIFIED key ("fedora.prod-cluster"), alongside
// (never replacing) any root-scoped entries — the gap deploy_ref.go's memory-documented spike
// identified in the resolved-project envelope (only rp.Templates.Local/Kubernetes/... at root scope).
func TestProjectTemplates_NamespaceQualified(t *testing.T) {
	...
	if _, ok := tpl.Kubernetes["prod-cluster"]; ok {
		t.Error("prod-cluster should NOT be visible at root scope (it's namespaced under 'fedora')")
	}
	body, ok := tpl.Kubernetes["fedora.prod-cluster"]
	if !ok {
		t.Fatalf("fedora.prod-cluster missing from the namespace-flattened Templates.Kubernetes map: %v", tpl.Kubernetes)
	}
	if len(body) == 0 {
...
// TestHostBuildDeployEntityResolve_KubernetesNamespaceQualified DELETED (K-wave W3a A3-phase-2): its

...
func TestResolveImage_QualifiedDelegates(t *testing.T) {
	...
	ri, err := resolveBoxTest(cfg, "sub.widget", "test", root, ResolveOpts{})
	if err != nil {
	...
	if _, err := resolveBoxTest(cfg, "app", "test", root, ResolveOpts{}); err != nil {
		t.Errorf("bare ResolveBox(\"app\") regressed: %v", err)
	}
	// A genuinely-missing namespace still errors clearly.
	if _, err := resolveBoxTest(cfg, "nope.widget", "test", root, ResolveOpts{}); err == nil {
...
func TestResolveAllImage_RequestedQualifiedTarget(t *testing.T) {
	...
	base, err := resolveAllBoxTest(cfg, "test", root, ResolveOpts{})
	if err != nil {
	...
	withReq, err := resolveAllBoxTest(cfg, "test", root, ResolveOpts{RequestedBoxes: []string{"sub.widget"}})
	if err != nil {

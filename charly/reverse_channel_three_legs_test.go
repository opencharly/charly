...
var reverseChannelHostBuilderWhitelist = []reverseChannelKindClause{
	...
	{"validate-project-checks", "validate_project_host.go", "M (CUE) + K1-loader-refs-IOU (KERNEL_MANIFEST.md W2 unit 3: hostBuildValidateProjectChecks's loadProjectForResolve path)"},
	...
	{"buildengine-collect-remote-refs", "host_build_buildengine.go", "K1-IOU (KERNEL_MANIFEST.md W2: hostBuildCollectRemoteRefs calls CollectRemoteRefsOpts, core-only per W1's config.go verdict)"},
	{"buildengine-ensure-repo", "host_build_buildengine.go", "B — bootstrap-delicate repo scan (KERNEL_MANIFEST.md W2: hostBuildEnsureRepo wraps EnsureRepoDownloaded, refs.go K1-floor)"},

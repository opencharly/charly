...
var reverseChannelHostBuilderWhitelist = []reverseChannelKindClause{
	...
	{"retention-defaults", "host_build_retention_defaults.go", "M — wire-broker leg, the retention-tunable read (KERNEL_MANIFEST.md: reads defaults.keep_images/keep_check_runs off charly.yml with no project walk and no network, so charly clean --dry-run stops hitting the 2m ceiling via LoadUnifiedViaExecutor's 70-call ref walk, issue #423)"},

...
// the loader stores a marshaled spec.Box per image name; consumers decode the authored BoxConfig
...
// boxMap is the generic image map type — name → opaque marshaled BoxConfig.
...
func (uf *UnifiedFile) BoxConfig(name string) (BoxConfig, bool) { return boxConfigFrom(uf.Box, name) }

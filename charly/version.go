...
// calver.ParsedCalVer is the parsed YYYY.DDD.HHMM calendar version. The parsed type + its parser
// live in the spec module's calver package (#55 value extraction) so BOTH core (the loader
// version gate) and the candy (the migration chain) reference the ONE copy. (The struct is named
